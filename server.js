...
if (!process.env.CLOUDINARY_CLOUD_NAME || !process.env.CLOUDINARY_API_KEY) {
  console.warn("Cloudinary credentials not set - image upload will fail")
}
...
app.get("/api/staff/inventory", authMiddleware, async (req, res) => {
  try {
    const items = await Inventory.find({ store_name: req.staff.store }).sort({ created_at: -1 })
    res.json(items)
  } catch (err) {
    res.status(500).json({ error: err.message })
  }
...
app.patch("/api/inventory/:id/stock", authMiddleware, async (req, res) => {
  try {
    const item = await Inventory.findOneAndUpdate(
      { _id: req.params.id, store_name: req.staff.store },
  ...
  const inventoryContext = inventory?.length
    ? `\n\nAVAILABLE STORE INVENTORY (suggest ONLY from these real products):\n${
        inventory.map((item, index) =>
          `${index + 1}. ${item.name} | Category: ${item.category} | Colors: ${item.colors.join(", ")} | Price: $${item.price} | ID: ${item._id || item.id}`
        ).join("\n")
...
setInterval(() => {
  https.get("https://classified-stylesense-ai.onrender.com/api/inventory", res => {
